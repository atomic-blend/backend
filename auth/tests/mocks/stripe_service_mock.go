...
func (m *MockStripeService) CreateCheckoutSession(ctx *gin.Context, customerID string, trialDays int64, successURL *string, cancelURL *string) (*stripe.CheckoutSession, error) {
	args := m.Called(ctx, customerID, trialDays, successURL, cancelURL)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockStripeService) CreateCustomerPortalSession(ctx *gin.Context, customerID string, returnURL *string) (*stripe.BillingPortalSession, error) {
	args := m.Called(ctx, customerID, returnURL)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*stripe.BillingPortalSession), nil
}
