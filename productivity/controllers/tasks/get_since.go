...
// GetTasksSince retrieves tasks updated since a specific date for the authenticated user with pagination
// @Summary Get tasks updated since date
// @Description Get tasks updated since a specific date for the authenticated user with pagination
// @Tags Tasks
// @Produce json
...
// @Router /tasks/since [get]
func (c *TaskController) GetTasksSince(ctx *gin.Context) {
	...
	// Get tasks updated since the specified time with pagination
	tasks, totalCount, err := c.taskRepo.GetSince(ctx, authUser.UserID, sinceTime, page, limit)
	if err != nil {
	...
	if tasks == nil {
		tasks = make([]*models.TaskEntity, 0)
	}
