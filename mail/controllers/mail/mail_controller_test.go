...
func TestMailController_GetAllMails(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "Success with all mails (no pagination)",
			queryParams:    "",
			...
			setupMock: func(mockRepo *mocks.MockMailRepository, userID primitive.ObjectID) {
				...
				mockRepo.On("GetAll", mock.Anything, userID, int64(0), int64(0)).Return(mails, int64(1), nil)
			},
		...
		},
		{
			...
			setupMock: func(mockRepo *mocks.MockMailRepository, userID primitive.ObjectID) {
				mails := []*models.Mail{}
				mockRepo.On("GetAll", mock.Anything, userID, int64(2), int64(5)).Return(mails, int64(0), nil)
			},
			setupAuth: func(c *gin.Context, userID primitive.ObjectID) {
				c.Set("authUser", &auth.UserAuthInfo{UserID: userID})
			},
		},
		{
			...
			setupMock: func(mockRepo *mocks.MockMailRepository, userID primitive.ObjectID) {
				mails := []*models.Mail{}
				mockRepo.On("GetAll", mock.Anything, userID, int64(0), int64(0)).Return(mails, int64(0), nil)
			},
			setupAuth: func(c *gin.Context, userID primitive.ObjectID) {
				c.Set("authUser", &auth.UserAuthInfo{UserID: userID})
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			{
				mailRoutes.GET("", controller.GetAllMails)
			}
			...
			} else if tt.name == "Invalid limit parameter" {
				mockRepo.AssertNotCalled(t, "GetAll", mock.Anything, userID, mock.Anything, mock.Anything)
			}
