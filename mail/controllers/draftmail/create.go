...
// CreateDraftMail creates a new draft mail entry
// @Summary Create a new draft mail
// @Description Create a new draft mail entry with the provided mail data
// @Tags DraftMail
// @Accept json
// @Produce json
// @Param body body CreateDraftMailRequest true "Draft mail data"
...
func (c *Controller) CreateDraftMail(ctx *gin.Context) {
	...
	log.Debug().Interface("raw_mail", rawMail).Msg("Received raw mail for draft")
	...
	for _, attachment := range encryptedMail.Attachments {
		...
		draftMail.Mail.Attachments = append(draftMail.Mail.Attachments, models.MailAttachment{
	...
	// Create draft mail with rollback on failure (s3 and mongo)
	createdDraftMail, err := c.draftMailRepo.Create(ctx, draftMail)
	if err != nil {
	...
	draftMail.ID = createdDraftMail.ID
	draftMail.CreatedAt = createdDraftMail.CreatedAt
	draftMail.UpdatedAt = createdDraftMail.UpdatedAt

	log.Debug().Interface("draft_mail", rawMail).Msg("Draft mail created successfully")

	ctx.JSON(http.StatusCreated, createdDraftMail)
}
