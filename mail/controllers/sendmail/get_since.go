...
// GetSendMailsSince retrieves send mails updated since a specific date for the authenticated user with pagination
// @Summary Get send mails updated since date
// @Description Get send mails updated since a specific date for the authenticated user with pagination
// @Tags SendMail
// @Produce json
...
func (c *Controller) GetSendMailsSince(ctx *gin.Context) {
	...
	// Get send mails updated since the specified time with pagination
	sendMails, totalCount, err := c.sendMailRepo.GetSince(ctx, authUser.UserID, sinceTime, int64(page), int64(size))
	if err != nil {
	...
	if sendMails == nil {
		sendMails = make([]*models.SendMail, 0)
	}
